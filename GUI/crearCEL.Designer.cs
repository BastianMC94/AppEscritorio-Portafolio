namespace GUI
{
	partial class crearCEL
	{
		/// <summary>
		/// Required designer variable.
		/// </summary>
		private System.ComponentModel.IContainer components = null;

		/// <summary>
		/// Clean up any resources being used.
		/// </summary>
		/// <param name="disposing">true if managed resources should be disposed; otherwise, false.</param>
		protected override void Dispose(bool disposing)
		{
			if (disposing && (components != null))
			{
				components.Dispose();
			}
			base.Dispose(disposing);
		}

		#region Windows Form Designer generated code

		/// <summary>
		/// Required method for Designer support - do not modify
		/// the contents of this method with the code editor.
		/// </summary>
		private void InitializeComponent()
		{
			this.lblClave = new MetroFramework.Controls.MetroLabel();
			this.lblSede = new MetroFramework.Controls.MetroLabel();
			this.lblCorreo = new MetroFramework.Controls.MetroLabel();
			this.txtSede = new MetroFramework.Controls.MetroTextBox();
			this.txtClave = new MetroFramework.Controls.MetroTextBox();
			this.txtCorreo = new MetroFramework.Controls.MetroTextBox();
			this.btnCrearCEL = new MetroFramework.Controls.MetroTile();
			this.SuspendLayout();
			// 
			// lblClave
			// 
			this.lblClave.Location = new System.Drawing.Point(108, 149);
			this.lblClave.Name = "lblClave";
			this.lblClave.Size = new System.Drawing.Size(81, 29);
			this.lblClave.TabIndex = 12;
			this.lblClave.Text = "Contraseña";
			this.lblClave.TextAlign = System.Drawing.ContentAlignment.MiddleRight;
			// 
			// lblSede
			// 
			this.lblSede.Location = new System.Drawing.Point(23, 79);
			this.lblSede.Name = "lblSede";
			this.lblSede.Size = new System.Drawing.Size(166, 29);
			this.lblSede.TabIndex = 7;
			this.lblSede.Text = "Sede";
			this.lblSede.TextAlign = System.Drawing.ContentAlignment.MiddleRight;
			// 
			// lblCorreo
			// 
			this.lblCorreo.Location = new System.Drawing.Point(108, 114);
			this.lblCorreo.Name = "lblCorreo";
			this.lblCorreo.Size = new System.Drawing.Size(81, 29);
			this.lblCorreo.TabIndex = 11;
			this.lblCorreo.Text = "Correo";
			this.lblCorreo.TextAlign = System.Drawing.ContentAlignment.MiddleRight;
			// 
			// txtSede
			// 
			// 
			// 
			// 
			this.txtSede.CustomButton.Image = null;
			this.txtSede.CustomButton.Location = new System.Drawing.Point(263, 1);
			this.txtSede.CustomButton.Name = "";
			this.txtSede.CustomButton.Size = new System.Drawing.Size(27, 27);
			this.txtSede.CustomButton.Style = MetroFramework.MetroColorStyle.Blue;
			this.txtSede.CustomButton.TabIndex = 1;
			this.txtSede.CustomButton.Theme = MetroFramework.MetroThemeStyle.Light;
			this.txtSede.CustomButton.UseSelectable = true;
			this.txtSede.CustomButton.Visible = false;
			this.txtSede.FontSize = MetroFramework.MetroTextBoxSize.Medium;
			this.txtSede.Lines = new string[0];
			this.txtSede.Location = new System.Drawing.Point(195, 79);
			this.txtSede.MaxLength = 12;
			this.txtSede.Name = "txtSede";
			this.txtSede.PasswordChar = '\0';
			this.txtSede.ScrollBars = System.Windows.Forms.ScrollBars.None;
			this.txtSede.SelectedText = "";
			this.txtSede.SelectionLength = 0;
			this.txtSede.SelectionStart = 0;
			this.txtSede.ShortcutsEnabled = true;
			this.txtSede.Size = new System.Drawing.Size(291, 29);
			this.txtSede.TabIndex = 8;
			this.txtSede.UseCustomForeColor = true;
			this.txtSede.UseSelectable = true;
			this.txtSede.WaterMarkColor = System.Drawing.Color.FromArgb(((int)(((byte)(109)))), ((int)(((byte)(109)))), ((int)(((byte)(109)))));
			this.txtSede.WaterMarkFont = new System.Drawing.Font("Segoe UI", 12F, System.Drawing.FontStyle.Italic, System.Drawing.GraphicsUnit.Pixel);
			// 
			// txtClave
			// 
			// 
			// 
			// 
			this.txtClave.CustomButton.Image = null;
			this.txtClave.CustomButton.Location = new System.Drawing.Point(263, 1);
			this.txtClave.CustomButton.Name = "";
			this.txtClave.CustomButton.Size = new System.Drawing.Size(27, 27);
			this.txtClave.CustomButton.Style = MetroFramework.MetroColorStyle.Blue;
			this.txtClave.CustomButton.TabIndex = 1;
			this.txtClave.CustomButton.Theme = MetroFramework.MetroThemeStyle.Light;
			this.txtClave.CustomButton.UseSelectable = true;
			this.txtClave.CustomButton.Visible = false;
			this.txtClave.FontSize = MetroFramework.MetroTextBoxSize.Medium;
			this.txtClave.Lines = new string[0];
			this.txtClave.Location = new System.Drawing.Point(195, 149);
			this.txtClave.MaxLength = 40;
			this.txtClave.Name = "txtClave";
			this.txtClave.PasswordChar = '\0';
			this.txtClave.ScrollBars = System.Windows.Forms.ScrollBars.None;
			this.txtClave.SelectedText = "";
			this.txtClave.SelectionLength = 0;
			this.txtClave.SelectionStart = 0;
			this.txtClave.ShortcutsEnabled = true;
			this.txtClave.Size = new System.Drawing.Size(291, 29);
			this.txtClave.TabIndex = 10;
			this.txtClave.UseCustomForeColor = true;
			this.txtClave.UseSelectable = true;
			this.txtClave.WaterMarkColor = System.Drawing.Color.FromArgb(((int)(((byte)(109)))), ((int)(((byte)(109)))), ((int)(((byte)(109)))));
			this.txtClave.WaterMarkFont = new System.Drawing.Font("Segoe UI", 12F, System.Drawing.FontStyle.Italic, System.Drawing.GraphicsUnit.Pixel);
			// 
			// txtCorreo
			// 
			// 
			// 
			// 
			this.txtCorreo.CustomButton.Image = null;
			this.txtCorreo.CustomButton.Location = new System.Drawing.Point(263, 1);
			this.txtCorreo.CustomButton.Name = "";
			this.txtCorreo.CustomButton.Size = new System.Drawing.Size(27, 27);
			this.txtCorreo.CustomButton.Style = MetroFramework.MetroColorStyle.Blue;
			this.txtCorreo.CustomButton.TabIndex = 1;
			this.txtCorreo.CustomButton.Theme = MetroFramework.MetroThemeStyle.Light;
			this.txtCorreo.CustomButton.UseSelectable = true;
			this.txtCorreo.CustomButton.Visible = false;
			this.txtCorreo.FontSize = MetroFramework.MetroTextBoxSize.Medium;
			this.txtCorreo.Lines = new string[0];
			this.txtCorreo.Location = new System.Drawing.Point(195, 114);
			this.txtCorreo.MaxLength = 40;
			this.txtCorreo.Name = "txtCorreo";
			this.txtCorreo.PasswordChar = '\0';
			this.txtCorreo.ScrollBars = System.Windows.Forms.ScrollBars.None;
			this.txtCorreo.SelectedText = "";
			this.txtCorreo.SelectionLength = 0;
			this.txtCorreo.SelectionStart = 0;
			this.txtCorreo.ShortcutsEnabled = true;
			this.txtCorreo.Size = new System.Drawing.Size(291, 29);
			this.txtCorreo.TabIndex = 9;
			this.txtCorreo.UseCustomForeColor = true;
			this.txtCorreo.UseSelectable = true;
			this.txtCorreo.WaterMarkColor = System.Drawing.Color.FromArgb(((int)(((byte)(109)))), ((int)(((byte)(109)))), ((int)(((byte)(109)))));
			this.txtCorreo.WaterMarkFont = new System.Drawing.Font("Segoe UI", 12F, System.Drawing.FontStyle.Italic, System.Drawing.GraphicsUnit.Pixel);
			// 
			// btnCrearCEL
			// 
			this.btnCrearCEL.ActiveControl = null;
			this.btnCrearCEL.Location = new System.Drawing.Point(470, 213);
			this.btnCrearCEL.Name = "btnCrearCEL";
			this.btnCrearCEL.Size = new System.Drawing.Size(75, 75);
			this.btnCrearCEL.Style = MetroFramework.MetroColorStyle.White;
			this.btnCrearCEL.TabIndex = 26;
			this.btnCrearCEL.TileImage = global::GUI.Properties.Resources.check_circle_o;
			this.btnCrearCEL.TileImageAlign = System.Drawing.ContentAlignment.MiddleCenter;
			this.btnCrearCEL.UseSelectable = true;
			this.btnCrearCEL.UseTileImage = true;
			this.btnCrearCEL.Click += new System.EventHandler(this.btnCrearCEL_Click);
			// 
			// crearCEL
			// 
			this.AutoScaleDimensions = new System.Drawing.SizeF(6F, 13F);
			this.AutoScaleMode = System.Windows.Forms.AutoScaleMode.Font;
			this.ClientSize = new System.Drawing.Size(568, 314);
			this.Controls.Add(this.btnCrearCEL);
			this.Controls.Add(this.lblClave);
			this.Controls.Add(this.lblSede);
			this.Controls.Add(this.lblCorreo);
			this.Controls.Add(this.txtSede);
			this.Controls.Add(this.txtClave);
			this.Controls.Add(this.txtCorreo);
			this.MaximizeBox = false;
			this.Name = "crearCEL";
			this.Resizable = false;
			this.Text = "Crear CEL";
			this.ResumeLayout(false);

		}

		#endregion

		private MetroFramework.Controls.MetroLabel lblClave;
		private MetroFramework.Controls.MetroLabel lblSede;
		private MetroFramework.Controls.MetroLabel lblCorreo;
		private MetroFramework.Controls.MetroTextBox txtSede;
		private MetroFramework.Controls.MetroTextBox txtClave;
		private MetroFramework.Controls.MetroTextBox txtCorreo;
		private MetroFramework.Controls.MetroTile btnCrearCEL;
	}
}